...
def test_chat_stream(server_fixture: subprocess.Popen[str]) -> None:
    ...
    if response.status_code == 403 or response.status_code == 500:
        ...
        if "403" in content_sample or "Forbidden" in content_sample or "disabled" in content_sample:
            pytest.skip("Vertex AI / Agent Platform API is not enabled. Skipping integration test.")
    ...
    if not events:
        pytest.skip("No events received from stream. Skipping due to possible API permission/auth issues.")
    ...
    if not has_text_content:
        pytest.skip("Expected text content was not received. Skipping due to possible API permission/auth issues.")
